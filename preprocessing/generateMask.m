...
%figure, imagesc(imSmooth1);
%figure, imagesc(imSmooth2);
...
figure, imagesc(mask_init);
se90 = strel('line',3,90);
se0 = strel('line',3,0);
BWsdil = imdilate(mask_init,[se90 se0]);
figure
imshow(BWsdil)
...
seD = strel('diamond',1);
maskFinal = imerode(BWdfill,seD);
%maskFinal = imerode(maskFinal,seD);
imshow(maskFinal)
...
% 
% maskBlobs=bwareaopen(mask_holes_filled,minCellSize);
...
% imshow(maskFinal)
% figure
% imshow(mask_holes_filled)
% figure
% imshow(maskBlobs)
% 
% %BWoutline = bwperim(maskFinal);
% Segout = image;
% %Segout(BWoutline) = min(image(:));
