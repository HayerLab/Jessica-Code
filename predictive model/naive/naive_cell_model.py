# -*- coding: utf-8 -*-
"""
Created on Tue Mar 19 13:20:42 2024
...
sys.path.append('/mnt/Z:/Jessica/tracking_code/factorial_hmm')
# import factorial_hmm
# from filterpy.kalman import KalmanFilter
import matplotlib.pyplot as plt

# load MATLAB file
data = scp.io.loadmat('/mnt/Z:/Jessica/tracking_code/data/varied_model/3_8_1_mask_tracking_varied_model')
filtered = scp.io.loadmat(
    '/mnt/Z:/Jessica/tracking_code/label_matrix/single_nuclei/data/3_8_1/labels/90_filtered_labels_consistent_3_8_1.mat')
...
cell_shape = np.nanmean(perim, 1) / np.sqrt(np.nanmean(area, 1))
...
features = np.hstack((cell_shape[filter_list], mean_ecc[filter_list], mean_circ[filter_list]))
X_train, X_test, y_train, y_test = train_test_split(features, mean_v[filter_list], test_size=0.20, train_size=0.80,
                                                    random_state=32)
...
mean_shape_seq = pd.DataFrame(np.nanmean(all_shape[filter_list, 0:179], 1))
...
for cell in range(len(filter_list)):
    for f in range(0, 179):
        ...
        if f > 1:
            ...
            shape_next = np.asarray(
                [df_shape.loc[cell].values[f], df_ecc.loc[cell].values[f], df_circ.loc[cell].values[f]])
