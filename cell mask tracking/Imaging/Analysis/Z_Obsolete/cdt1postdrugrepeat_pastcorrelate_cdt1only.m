function [goodfirstmitosis,goodg1length,goodg1lengthlineage,goodsg2length,goodangieg1,goodangiemean] = cdt1postdrugrepeat_pastcorrelate_cdt1only(row,col,bmax)
...
maxfirstmitosis = (40-bmax)*5;   %was 24
g1window = [9 10];
%%%%%%%%%%%%%% viewer settings %%%%%%%%%%%%%%%%%%
...
[signal1,mitoses] = combinedata_realbirths_cdt1only(row,col,site,path);
...
for i=1:samplesize
    ...
    while mitosesafterspike(i,k)
        ...
        boundaryflag(i,k) = truemax(cdt1(i,:),cdt1timeactual(i,k),numframes,startpoint,'left');
