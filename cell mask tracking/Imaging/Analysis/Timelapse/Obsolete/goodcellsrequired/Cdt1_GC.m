...
if stairview || scatterview
    ...
    %set(stairfig,'Position',[round(0.4*screenx) round(0.2*screeny) round(0.3*screenx) round(0.6*screeny)]); %3x1
    %set(stairfig,'color','w','PaperPosition',[0 0 4 14]); %cumulative stairs (3x1)
    set(stairfig,'Position',[round(0.2*screenx) round(0.2*screeny) round(0.6*screenx) round(0.3*screeny)]); %1x3
    set(stairfig,'color','w','PaperPosition',[0 0 14 4]); %cumulative stairs (1x3)
...
for i = 1:condnum
    ...
    %values = angiemean(:);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [valuesfm,idx]=sort(valuesfm);
    values=values(idx);
    avgvals=zeros(size(values));
    fmbin=floor(valuesfm/3);
    fmbinunique=unique(fmbin);
    for vc=fmbinunique'
        vcidx=find(fmbin==vc);
        vcavg=mean(values(vcidx));
        avgvals(vcidx)=vcavg;
    end
    ...
    if scatterview
        subaxis(1,4,i,'SV',0.05,'ML',0.05,'MR',0.05,'PB',0.05); %Cdt 1x2
        %subaxis(1,3,i,'SV',0.05,'ML',0.05,'MR',0.05,'PB',0.05); %Cdt 1x3
        ...
        scatter(valuesfm,values,'.');
        hold on;
        %line(valuesfm,filtfilt(filtfiltvec,1,values),'Color','r','Linewidth',2);
        scatter(valuesfm,avgvals,'.r');
