function bg_img=calculate_bg_img_rm_blobs(filepath,bgfilepath,filenamecontains)
...
filenameStructure = ['*_',filenamecontains,'*.tif'];
files=getfilenames(filepath,filenameStructure);
%files=files(boolRegExp(files,filenamecontains));

imstack_raw=[];
for i=1:length(files)
    tempim_raw=double(imread([files{i}]));
...
% imagesc(bg_img);
imwrite(bg_img,[bgfilepath,filesep,'AVG_bg',filenamecontains,'.tif'],'tif','Compression','none');
