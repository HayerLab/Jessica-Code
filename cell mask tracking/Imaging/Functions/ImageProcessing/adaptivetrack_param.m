function [tracedata,curdatatracked,tracking,nuc_label]=adaptivetrack(lastgoodframe,curframe,tracedata,curdata,tracking,nuc_raw,nuc_label,nucr,jitter,H2BvsNLS,debugpackage)
...
circcur = curdata(:, 5); eccencur = curdata(:,6); perimcur = curdata(:,7);
...
%%% update tracked info %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
curdatatracked=ones(maxcellnum,7)*NaN;
relabelidx=ones(numcur,1)*NaN;
%%% add tracked cells %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
matchprevidx=find(~isnan(prevmatch));
matchcuridx=prevmatch(matchprevidx);
curdatatracked(matchprevidx,:)=[xcur(matchcuridx) ycur(matchcuridx) areacur(matchcuridx) masscur(matchcuridx) circcur(matchcuridx) eccencur(matchcuridx) perimcur(matchcuridx)];
...
if numdaughter>0
    ...
    curdatatracked(daughterpx,:)=[xcur(daughtercx) ycur(daughtercx) areacur(daughtercx) masscur(daughtercx) circcur(matchcuridx) eccencur(matchcuridx) perimcur(matchcuridx)];
...
if nummerge>0
    ...
    curdatatracked(mergepx,:)=[xcur(mergecx) ycur(mergecx) areacur(mergecx) masscur(mergecx) circcur(matchcuridx) eccencur(matchcuridx) perimcur(matchcuridx)];
...
if numnonmatched>0
    ...
    curdatatracked(nonmatchpx,:)=[xcur(nonmatchcx) ycur(nonmatchcx) areacur(nonmatchcx) masscur(nonmatchcx) circcur(nonmatchcx) eccencur(nonmatchcx) perimcur(nonmatchcx)];
