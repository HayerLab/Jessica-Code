...
for f=framezero+155
    ...
    %%% image processing %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    blocknum=1;
    DAs_bs=bgsub_MC(DAs_or,10*nucr,0.05);
    REs_ma=ThreshImage_MC(REs_or,0);
    bg_or=REs_or.*~REs_ma;%bgmax=max(max(bg_or));bgs=bg_or(:);bgmin=min(bgs(bgs>0));
    [height,width]=size(DAs_or);
    blockheight=ceil(height/blocknum);
    blockwidth=ceil(width/blocknum);
    bg_nan=bg_or;
    bg_nan(bg_nan==0)=NaN;
    bgblock=blockproc(bg_nan,[blockheight blockwidth],fun);
    bgfinal=imresize(bgblock,size(DAs_or),'bicubic');
    REs_bs=REs_or-bgfinal;
    REs_bs(REs_bs<0)=0;
    
    %REs_bs=REs_bs.*REs_ma;
    %%% extract features %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    DAs_pad=getnucmask(DAs_bs,nucr);
    [DAs_da,finalcytoring,realnuc_la,legitedges]=buildcytoring_test(DAs_pad,REs_bs,nucr);
    fcr_da=regionprops(finalcytoring,REs_bs,'MeanIntensity','PixelIdxList');
    ...
    %cytonuc_la(ismember(cytonuc_la,nucexclude))=0;
    finalcytoring(ismember(finalcytoring,nucexclude))=0;
    
    %%% Extract relevant ring range %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    numcells=size(DAs_da,1);
    for k=1:numcells
        [nout,xout]=ksdensity(REs_bs(fcr_da(k).PixelIdxList));
    end
    
    %%% only for visualization %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %cytoringperi=bwmorph(cytoring,'remove');
    %cytoringperi=segmentedringmap;
    %segmentedringmap=segmentedringmap & cytoring;
    nucmoviering=bwmorph(realnuc_la,'remove');
    cytomoviering=logical(finalcytoring);
    %cytoringperi=segmentedringmap;
    %cytoringperi=segmentedringmap | N_out | NE_out | E_out | SE_out | S_out | SW_out | W_out | NW_out;
    %cytoringperi=cytoringouter;
    %cytoringedges=legitedges;
    ...
    if movie
        %{
        tempframe=imadjust(mat2gray(nucmoviering));
        tempframe(:,:,2)=imadjust(mat2gray(REs_bs));
        ...
        %bgmin=min(min(REs_bs));bgmax=max(max(Imagebg));
        %fprintf('range of Imagebg = %0.0f to %0.0f\n',bgmin,bgmax);
