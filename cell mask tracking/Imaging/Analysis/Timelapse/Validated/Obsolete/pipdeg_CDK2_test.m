...
for i=1:numtraces
    earlyval(i)=alltracesCdk2(i,alldaughterstats(i,1)+(minlengthdaughter/2)-1); %Sabrina uses 2hr
    lateval(i)=alltracesCdk2(i,alldaughterstats(i,1)+minlengthdaughter);
    maxval(i)=max(alltracesCdk2(i,alldaughterstats(i,1)+(minlengthdaughter/2)-1:alldaughterstats(i,2))); %max from minlength-end
end
%figure,hist(minval,100);
mincutoff=0.60; %minval at 20frames
latecutoff=0.9;
maxcutoff=0.60; %maxval cutoff F20-end
Cdk2inc=earlyval>mincutoff & lateval>latecutoff; %default 0.4
Cdk2low=earlyval<mincutoff & lateval<maxcutoff;
