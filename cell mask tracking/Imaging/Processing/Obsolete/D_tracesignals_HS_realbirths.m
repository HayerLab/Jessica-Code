...
sample = [1424];
%sample = [8192];           % 34 in MC that is same as 779 in FTC
...
if registry
    load([datadir,moviename,'_alldata_HS_OP'],'bestsp','best_rc','corruptlist','leaveoutlist','blurframes')
...
for i = leavein
    ...
    daughtermitoses(find(daughtermitoses==best_rc(i,1))) = [];          %remove daughter splits at real birth
    goodmitoses(i,m:m+length(daughtermitoses)-1) = daughtermitoses;
    if m>1
        goodmitoses(i,find(goodmitoses(i,:)==best_rc(mother,1))) = 0;   %remove mother's birth
    end
end
...
for i = leavein
    ...
    for tt = 1:scanwidth
        ...
        nucsigdrop = [nucsigdrop,tracklengthmod(diff(nucsig(i,tracklengthmod))<-6)]; %try out several to maximize passes
    end
...
for counter=1:length(tc_sel)
    ...
    if screening
        ...
        selection = enoughhistory & ~selectedout & selectedin & ~corrupted  & ~errantmitosis;
