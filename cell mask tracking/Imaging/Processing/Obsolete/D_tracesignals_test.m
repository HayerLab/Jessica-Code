%function D_tracesignals(row,col,site)
...
%%%%%%%%%%%% Experiment Settings %%%%%%%%
framesperhr = 5;
drugtime = 40;          %set to zero if no drug
%%%%%% User settings %%%%%%%%%%%%%%%%%%%%
selectmode = 0;         %1=enable imaging and selection. 0=viewing traces only
sample = [];
%sample = [526 528 497 559 561 563 566 575 580 500 434 427 284 366 328 247 252 256 266 271 150 176 210 214 225 209 80 101 109 144 147];
%sample = [528 488 506 522 328 350 459 268 271 152 427 366 566 250 283 124 80]; %fall
%sample = [];
%selectin = [526 528 497 561 563 566 575 580 500 434 427 284 366 328 247 252 256 266 271 150 176 210 214 225 209 80 101 109 144 147];
...
selectin = [44 80 101 109 271 427 488 500 506 511 532 576 592 598 603 626 661 662 763 849];
...
plotsignal3 = 0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%ymin1 = 0.2; ymax1 = 2.5; ystep1 = 0.2;   %ymax1=2
ymin1 = 0; ymax1 = 2.5; ystep1 = 0.05;   %log calcs
%ymin1 = 50; ymax1 = 100; ystep1 = 100;
%ymin1 = 0; ymax1 = 500; ystep1 = 100;

ymin2 = 0; ymax2 = 0.5; ystep2 = 0.05; %log(cdt1)
%ymin2 = 0; ymax2 = 10; ystep2 = 1;
%ymin2 = 1; ymax2 = 2; ystep2 = .1;     %nuclear signal (log(H2B))
%ymin2 = 0; ymax2 = 10000; ystep2 = 1000;
%historycutoff = 0.5;
historycutoff = 1;
screening = 1;
dotsize = 8;
...
if registry
    load([datadir,moviename,'_alldata_HS_L05_595OfSegs_outer2'],'bestsp','best_rc','corruptlist','leaveoutlist')
    %load([datadir,moviename,'_alldata_FCT'],'bestsp','best_rc','corruptlist','leaveoutlist','blurframes')
...
for f = 1:size(signal1,2)
    ...
    %signal1(tempcell,f) = bestsp{f}(tempcell,7);
    %signal1(tempcell,f) = bestsp{f}(tempcell,4)/200;
    %signal1(tempcell,f) = bestsp{f}(tempcell,3);
    %signal2(tempcell,f) = bestsp{f}(tempcell,3);
...
for i = leavein
    ...
    for tt = 1:scanwidth
        ...
        nucsigdrop = [nucsigdrop,tracklengthmod(diff(nucsig(i,tracklengthmod))<-0.33)]; %try out several to maximize passes. default=-0.33
    end
...
for counter=1:length(tc_sel)
    ...
    if screening
        ...
        %selection = enoughhistory & ~corrupted & ~errantmitosis;
    ...
    if selection
        ...
        if selectmode
            ...
            if sample
                ...
                %subplot(4,5,mod(j-1,24)+1);
                subaxis(4,5,mod(j-1,24)+1,'ML',0.02,'MR',0.01,'MT',0.03,'MB',0.03,'SH',0.02); %5x4
            end
        end
        set(gcf,'color','w');
