...
allxorg=zeros(totalcells,totalframes); allyorg=allxorg;
for f=1:totalframes
...
if start
    ...
    load([savedir,shot,'_goodcells.mat'],'tracelist');
...
for counter=1:length(tc_sel)
    ...
    if manualgating
        ...
        save([savedir,shot,'_goodcells.mat'],'tracelist');
    end
end
fprintf([num2str(j),'\n']);
if directgating
    tracelist = savetracelist(find(savetracelist));
    save([savedir,shot,'_goodcells.mat'],'tracelist');
    save([savedir,shot,'_singletracedata.mat'],'signal1','signal2','goodmitoses','realbirths');
end
%{
