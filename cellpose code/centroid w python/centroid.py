# -*- coding: utf-8 -*-
"""
Created on Mon Oct 30 12:20:16 2023
modified from https://github.com/MouseLand/cellpose/issues/496
move masks to one directory using zsh command cp **/*.npy ./all_masks
"""
import pickle
import pandas as pd
import scipy
import numpy as np
import cellpose
import natsort
from os import listdir
import matplotlib.pyplot as plt
from datetime import datetime
import gc
# Function for calculating centerpoints (again)
print(datetime.now())
def calculate_centers(masks):
    slices = scipy.ndimage.find_objects(masks)
    centroids = list()
    centroids_x = list()
    centroids_y = list()
    for i, elem in enumerate(slices):
        if elem is not None:
            sr, sc = elem
            mask = masks[sr, sc]
            center = scipy.ndimage.center_of_mass(mask)
            #print(center)
            #center_rounded = (round(center[0]) + sr.start, round(center[1]) + sc.start)
            centroids_x.append(center[0])
            centroids_y.append(center[1])
            centroids.append(center)
    #return centroids_x, centroids_y
    return centroids_x, centroids_y
#directory = ["E:\\Jessica\\segmentation\\3_7_1\\Composites\\aligned\\export_fig_tiff\\all_masks\\sub\\"]
directory = ["E:\\Jessica\\segmentation\\3_7_1\\Composites\\aligned\\export_fig_tiff\\all_masks\\", "E:\\Jessica\\segmentation\\3_8_1\\Composites\\aligned\\", "E:\\Jessica\\segmentation\\3_8_2\\Composites\\aligned\\"]
shots = ["3_7_1","3_8_1", "3_8_2"]
# function for loading information and calculate center positions
def get_centroid(directory):
    gc.collect() #garbage collection to free up memory
    data =dict()
    masks = dict()
    centers = dict()
    image = dict()
   
    for shot in directory:
        print(shot)
        k = 0
        #shot_data = np.empty()
        shot_data = list()
        sorted_dir_content = natsort.natsorted(listdir(shot))
        for content in sorted_dir_content:
            if content.split('.')[-1] == "npy":
                print(k)
                #load_data = np.load(shot+content, allow_pickle=True)
                shot_data.append(np.load(shot+content, allow_pickle=True).item())
                #del load_data
                gc.collect()
                #load_data.close()
                k+=1
            #del content 
        data[shot] = shot_data
    #data = {shot:  for shot in directory for content in listdir(shot) if content.split('.')[-1] == "npy"}
    frames_mask = dict()
    frames_img = dict()
    value = data.values()
   
    for key,value in data.items():
    #for timepoint in value[idx]:
        #if idx in range(len(directory)):
            masks_timeseries = list()
            image_series = list()
            for timepoint in value:
                masks_timeseries.append(timepoint['masks'])
                if key != 'E:\\Jessica\\segmentation\\3_7_1\\Composites\\aligned\\export_fig_tiff\\all_masks\\':
                        image_series.append(timepoint['img'])
                masks[key] = masks_timeseries
                image[key] = image_series
               # idx+=1
        #else: break
    
    center_df_shots = dict()
    
    for key,value in masks.items():
        center_df_frames = dict()
        center_timepoint_x = list()
        center_timepoint_y = list()
        columns = list(range(len(value)))
        df_x =  pd.DataFrame(columns=columns)
        df_y = pd.DataFrame(columns=columns)
        for index, elem in enumerate(value):
            x, y = calculate_centers(elem)
            #center_timepoint_x.append(center)
            #center_timepoint_y.append(center_y)
        #centers[key] = [center_timepoint_x,center_timepoint_y]
            if index == 0: 
                df_x = pd.DataFrame(x, columns=[index])
                df_y = pd.DataFrame(y, columns=[index])
            else:
                df_x_t = pd.DataFrame(x, columns=[index])
                df_y_t = pd.DataFrame(y, columns=[index])
                #df_x[index] = x
                #df_y[index] = y
                df_x = pd.concat([df_x, df_x_t],axis=1)
                df_y = pd.concat([df_y,df_y_t], axis=1)
        #center_df_frames[index] = [df_x, df_y]
       # center_df_shots[key] = center_df_frames
        center_df_shots[key] = [df_x, df_y]
    return data,masks,image,center_df_shots

data,masks,image,centroid = get_centroid(directory)
print(datetime.now())
index = 0
for k,v in centroid.items(): 
    for i,coord in enumerate(v):
        if i == 0:
            coord.to_csv("E:\\Jessica\\segmentation\\centroid\\Centroid"+shots[index]+"_x.csv")
        else: 
            coord.to_csv("E:\\Jessica\\segmentation\\centroid\\Centroid"+shots[index]+"_y.csv")
    index+=1
