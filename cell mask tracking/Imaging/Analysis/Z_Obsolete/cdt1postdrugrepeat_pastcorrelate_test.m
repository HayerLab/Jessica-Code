...
for i=1:samplesize
    ...
    while mitosesafterspike(i,k)
        ...
        boundaryflag(i,k) = truemax_prev(cdt1(i,:),cdt1timeactual(i,k),numframes,startpoint,'left');  %combinedata only keeps unique data after realbirth, so this may need to be changed
        %%%%%%%%%%% calculate following G1 length & activity %%%%%%%%%%%%%%
        if k==1    %due to time restrictions, can only consider 1 round
            prevpoint = mitosesafterspike(i,k);
            ...
            boundaryflagnext(i,k) = truemax(g1length(i)+mitosesafterspike(i,k),prevpoint,nextpoint);
            ...
            if prevexists
                ...
                boundaryflagprev(i) = truemax(g1lengthprev(i)+prevmitosis,prevmitosis,drugspike);
            end
        end
        %%%%%%%%%%% reset for next loop %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
leavein(logical(boundaryflagnext)) = 0;  %boundaryflag=1 if max cdt1 doesn't drop off on both sides
