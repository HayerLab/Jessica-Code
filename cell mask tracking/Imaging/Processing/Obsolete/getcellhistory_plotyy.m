...
for counter=1:length(tc_sel)
    ...
    if selection
        ...
        set(gcf,'color','w');
        %axis([1,totalframes,ymin1,ymax1]);
        ...
        %    line(x1,y1,'LineStyle','--','DisplayName',num2str(cellid),'linewidth',signalwidth);
        %    line(x2,y2,'DisplayName',num2str(cellid),'linewidth',signalwidth);
        %    x = [x1,x2];
        %    ysig1 = [y1,y2];
        %else
        ...
        %end
        %set(gca,'XTick',0:1:totalframes/framesperhr);
        %xlim([0 totalframes/framesperhr]);
        xtime = x/framesperhr;
        if plotsignal2
