...
%row=2;col=1;site=1; %2 2 4
%%% set paths %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
projectpath='D:\data\140708_Axon\';
imagepath='D:\data\140708_Axon\';
%shadingpath='H:\Images\ShadingImages\20140506 CYT 10x Long 200ul\';
experimentpath='Mitofission\';
...
moviebin=1;
if moviebin==1
    nucr=4; % nucr=4 for IXµ, 4x 
    debrisarea=10; % =20 for IXµ 4x
    %nucr=20;
    %debrisarea=600;
elseif moviebin==2
    nucr=6;
...
jitters=zeros(EF,2);
blocksize=20000; %% AH changed form 10000 140703, crashed in line 180 otherwise
maxcellnum=blocksize;
tracedata=ones(maxcellnum,EF,4+ringcalc)*NaN; %default 7
...
%     raw3=single(imread([rawdir,name3,num2str(f),'.tif'])); %raw3=(raw3-bgcmos)./sc3;
    ...
    nanmask=imdilate(foreground,strel('disk',floor(nucr/2)));
...
%         sig3(n)=median(real3(nuc_info(cc).PixelIdxList));
    ...
    if maxcellnum-max(cellid)<3000
        tempdata=ones(blocksize,EF,4+ringcalc)*NaN; %default 7
