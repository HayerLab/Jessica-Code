...
G2length=(numframes-degends)/framesperhr;
G2sample=~isnan(degstarts) & ~isnan(degends) & G2length<=5;
...
%%% G2 timelapse %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure, hold on;
scatterandfit_definebins(G2time(G2inc),G2IF(G2inc),'b','b',0.5:1:4.5);
scatterandfit_definebins(G2time(G2low),G2IF(G2low),'r','r',0.6:1:4.6);
...
title('Cyclin D1 levels in G2 phase'); xlabel('Cdk2-inc vs Cdk2-low)'); ylabel('Cyclin D1 (log(median-RFU))');
set(gcf,'color','w','PaperPosition',[0 0 3 3]); saveas(gcf,'h:\Downloads\Fig2.jpg');
%%% S and G2 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
