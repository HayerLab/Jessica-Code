...
%row=2;col=10;site=1; %2 2 4
%%% set paths %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
projectpath='N:\';
imagepath='N:\';
experimentpath='161103_IXM\raw\stitched\';
%bgpath='L:\data\160308_IX\160308-IXM\background';
% projectpath='D:\data\';
% imagepath='D:\data\';
% experimentpath='\140710-IX\Y27632\';
...
if ~exist([datadir,'tracedata_',shot,'.mat'],'file'); % makes sure analsis is run only for wells that haven't been analyzed yet
    ...
    if moviebin==1
        nucr=14;
        debrisarea=400; %MCF-10A 10xBin1: H2B:100 NLS:200 !!! Threshold-only objects larger are considered nuclei
        %nucr=20;
        %debrisarea=600;
    elseif moviebin==2
        nucr=6;
    ...
    tracedata=ones(maxcellnum,EF,4)*NaN; %default 7
...
%     sc1=double(imread([bgpath,filesep,'AVG_bgDAPI.tif']))-cmosoffset;
    ...
    %     real3=bgsubmasked_4(blur3,nanmask,1,compression);
        ...
        blurthreshhigh=1.5*mednuc;%1.1*mednuc;  %H2B:1.1 NLS:1.08
        blurthreshlow=0.5*mednuc;%0.8*mednuc;   %H2B:0.8 NLS:0.95
        ...
        if maxcellnum-max(cellid)<blocksize
            tempdata=ones(blocksize,EF,4)*NaN; %default 7
