...
% minthresh=30; %threshold below which min of each trace must be
% [traces2,badtraces2]=gate_lengthandrange(tracedata,tracestats,channelPIP,minlengthtrace,maxthresh,minthresh);
...
%rangeoption=1; %1:normalize by median of max of all traces if min>max*0.9
%traces2=normalizetraces_2(traces2,rangeoption);
%%% categorize traces %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%POIcalc=POI_Cdk2-tracestats(:,1);
POIcalc=tracestats(:,3);
%%% gate POI calculations %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
badtracesallPOI=zeros(size(POIcalc,1),1);
...
alignPOI=tracestats(:,1); %absolute frame to align by (will be set to '0' on x-axis)
%alignPOI=ones(size(tracesgated,1),1)*drugspike;
[alignedtime,aligneddata]=aligntraces_2(tracesgated,alignPOI,tracestats,motherstats); alignedtime=alignedtime/framesperhr;
%%% plot traces %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
