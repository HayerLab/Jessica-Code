function timelapsesetup3(row,col,site,mask_list,path)
%row=3;col=5;site=1; %2 2 4
%%% set paths %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
projectpath='E:\Jessica\CellTracking2018';
imagepath='E:\Jessica\CellTracking2018';
experimentpath='\sample data\';
files_mask= dir(fullfile('E:\Jessica\segmentation\3_8_1\Composites\aligned\csv', '*.csv'));
files_mask = struct2cell(files_mask);
files_mask = files_mask(1,:);
mask_list = files_mask;
path = 'E:\Jessica\segmentation\3_8_1\Composites\aligned\csv';
...
if ~exist([datadir,'tracedata_',shot,'.mat'],'file'); % makes sure analsis is run only for wells that haven't been analyzed yet
    ...
    if separatedirectories==1
        ...
        rawdir=[imagepath,experimentpath,shot,'_']; %AH
    ...
    if moviebin==1
        nucr=4;
        debrisarea=40; %MCF-10A 10xBin1: H2B:100 NLS:200 !!! Threshold-only objects larger are considered nuclei
        %nucr=20;
        %debrisarea=600;
    elseif moviebin==2
        nucr=6;
    ...
    %name2='YFP_';
    %name3='RFP_';
    namenucedge=[shot,'_nucedge_'];
    adjustframe=0; %default

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    frames=1:length(mask_list);
    totalframes=numel(frames);
    badframes=ones(length(mask_list),1)*NaN;
%     if SF>1
%         badframes(1:SF-1)=0;
%     end
    jitters=zeros(length(mask_list),2);
    blocksize=1000; %% AH changed form 10000 140703, crashed in line 180 otherwise
    maxcellnum=blocksize;
    tracedata=ones(maxcellnum,length(mask_list),4)*NaN; %default 7
    ...
    for i=1:totalframes
       ...
       % nuc_mask=blobdetector_3(log(raw1),nucr,blobthreshold,debrisarea);
        nuc_mask = readmatrix(strcat(path,'\', mask_list{1}));
        foreground=nuc_mask;
%         if i==firstgoodindex
...
%         end
        %replace code here with csv of cellpose masks 
        %%% remove border objects %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         height =size(nuc_mask,2);
%         width = size(nuc_mask,1);
        nuc_mask([1 50],1:50)=1; 
        nuc_mask(1:50,[1 50])=1;
%         border=bwareaopen(nuc_mask,100,4);
%         nuc_mask=logical(nuc_mask-border);
        ...
        compression=4;
        %real1=bgsubmasked_3(raw1,nanmask,1);
    %     blur2=imfilter(raw2,fspecial('disk',3),'symmetric');
    %     real2=bgsubmasked_4(blur2,nanmaskcyto,1,compression);
    %     blur3=imfilter(raw3,fspecial('disk',3),'symmetric');
    %     real3=bgsubmasked_4(blur3,nanmask,1,compression);
        %%% extract features %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        [nuc_label,numcells]=bwlabel(nuc_mask);
        nuc_info=struct2cell(regionprops(nuc_mask,'Area','Centroid','Circularity','Eccentricity','Perimeter')');
        %dimensions: parameter # x # of frames X # of cells 
        nuc_area=squeeze(cell2mat(nuc_info(1,1,:)));
        %%%%%% detect bad frame %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        mednuc=median(nuc_area);
%         if i>1 && (numcells<numthresh || mednuc>blurthreshhigh || mednuc<blurthreshlow)   
...
%         end
        ...
        disp(size(nuc_center))
        nuc_circular=squeeze(cell2mat(nuc_info(3,1,:)));
        nuc_eccentric=squeeze(cell2mat(nuc_info(4,1,:)));
        nuc_perimeter=squeeze(cell2mat(nuc_info(5,1,:)));
        disp(size(nuc_center))

        %%% calculate masses %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       % nuc_mass=nuc_density.*nuc_area;
        curdata=[nuc_center(1,:)',nuc_center(2,:)',nuc_area,nuc_circular, nuc_eccentric, nuc_perimeter]
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if i>firstgoodindex
            %%% calculate jitter %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%             nuc_center(:,2)=nuc_center(:,2)+jitters(f,2);
            ...
            [tracedata,curdata,tracking,nuc_label]=adaptivetrack_5modified(i-1,f,tracedata,curdata,tracking,nuc_label,zeros(2),20,1);
           % badframes(f)=0;
        end
        ...
        if ringcalc==1
            ...
            ring_label=getcytoring_3(nuc_label,2,real2); %10xB1:4 20xB1:4
        ...
        if maxcellnum-max(cellid)<blocksize
            tempdata=ones(blocksize,length(mask_list),4)*NaN; %default 7
