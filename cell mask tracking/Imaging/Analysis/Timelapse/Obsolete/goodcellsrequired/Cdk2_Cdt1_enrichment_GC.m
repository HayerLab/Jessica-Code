...
drugspike = 1;
framesperhr = 5;
measurelimit = 34;    %this is the longest measurement to consider
...
if stairview || timeview || correlationview || timepercentview || enrichmentview
    stairfig = figure;
...
for i = 1:condnum
    ...
    if stairview
        ...
        g1risemeasured = 0;     %set to 1 if measuring G1 rise length
        if g1risemeasured
    ...
    if enrichmentview
        ...
        c_elements(end) = c_elements(end)+quiescentcount;     %so we can represent the quiescent cells
        c_elements = cumsum(c_elements);
        less_elements = [0;c_elements(1:end-1)];
        remaining_elements = samplesize(i)-less_elements;
        enrichment = 100*n_elements./remaining_elements;
        versus = 1;
        if versus
            if i==1
                control = enrichment;
            else
                subaxis(2,2,i-1,'ML',0.1,'MR',0.03,'MT',0.05,'MB',0.1);
                gray = [0.65 0.78 0.78];
                %plot(bin,control,'-ko','linewidth',2,'markeredgecolor',gray,'markerfacecolor',gray,'markersize',6); hold on;
                plot(bin,control,'color',gray,'linewidth',2); hold on;
                plot(bin,enrichment,'-mo','linewidth',2,'markeredgecolor','k','markerfacecolor',[.49 1 .63],'markersize',8);
                for p=1:length(bin)
                    text(bin(p)-1,enrichment(p)+2,num2str(remaining_elements(p)));
                end
                namecount = [char(conditions(i,1)),' (n=',mat2str(samplesize(i)),')'];
                title(namecount);
            end
        else
            subaxis(1,1,i,'ML',0.1,'MR',0.03,'MT',0.02,'MB',0.1);
            plot(bin,enrichment,'-mo','linewidth',2,'markeredgecolor','k','markerfacecolor',[.49 1 .63],'markersize',10); hold on;
            for p=1:length(bin)
                text(bin(p)-1,enrichment(p)+1,num2str(remaining_elements(p)));
            end
            namecount = [char(conditions(i,1)),' (n=',mat2str(samplesize(i)),')'];
            legend(char(namecount),'location','northeast');
        end
        xlim([bmin bmax-bstep]);
        ylim([0 20]);
    end
