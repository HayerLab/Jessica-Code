function Timelapse_4_gatetraces(row,col,site)
%row=1;col=11;site=1;
%row='D';col='05';site='4';
...
global path moviename allx ally allxorg allyorg plotfignum immunoframe
...
sample = [];
%sample = [49 76 116 145 21];
selectin = [];
%selectin = [21 49 76 116 145];

%C_05_1
%selectin = [85 103 298 365 391 399 445 459 468 696 825 848 897 1061 1132 1356 1532 1562 1660 2150 2376 2446];
%selectin = [85 103 275 317 399 407 445 468 825 897 1356 1532 1818 1660 2446 3439];


%D_05_1
%normals
%selectin = [28 39 55 159 265 315 332 376 632 1393 2167 2296 2603 2670 2680 3315];
%CDK4i pop
%selectin = [22 44 46 55 126 158 291 304 332 1375 3742 3972];
%early pop
%selectin = [109 115 201 254 261 376 2264 2296 2375 2670 1393
%G0 (late) pops
%selectin = [];
%trough
%selection = [3 39 230 328 340 1033 1058 1125 1238 3159 3315 3644]
%false S-phase
%selectin = [56 388 1614 1946]
%smalls
%selectin = [28 265 328 376 632 1055 1393 1674 1892 2167 2296 2603 2670 2680 3306]

%E_05_1
%normal
%data%selectin = [104 148 239 276 316 390 745 779 920 998 1121 1222 1330 1949 1961 2011];
%selectin = [415 941 1245 1606 1934 1952 1998 2184 2619 2703 3734 3827 3849 3933];
%slow fall
%selectin = [2862];
%CDK4i pop
%data%selectin = [69 95 74 122 323 1040 1037 1040 2011];
%selectin = [2861];
%G0 pop
%selectin = [211 261 287 288 323 996 1043 1049 1086 2190 2246];
%no normal pop
%selectin = [306 1934 1952 1998 2185 3609 3734 3808 3827 3933];
%early pop
%selectin = [3 59 81 944];
%toxic
%data%selectin = [21 61 69 74 122 207 209 212 213 227 267 269 299 304 334 1391]; 
...
immunostain = 1;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ymin1 = 0.3; ymax1 = 2; ystep1 = 0.3;   %DHB ratio (cells w/ geminin)
%ymin1 = 0; ymax1 = 1.5; ystep1 = 0.3; %S-phase sensor
%ymin1 = 0; ymax1 = 1000; ystep1 = 100;

%ymin2 = 0; ymax2 = 1.5; ystep2 = 0.3; %most sensors
...
if registry
    load([datadir,moviename,'_alldata',stringIF],'bestsp','best_rc','corruptlist','leaveoutlist','badframes')
    %fprintf('bad frames: \n'); disp(badframes);
else
    load([datadir,moviename,'_alldata',stringIF'],'bestsp','best_rc')
end

load([datadir,'jitter_',IFstring,moviename, '.mat'],'x','y');
...
for f = 1:size(signal1,2)
    ...
    %signal1(tempcell,f) = bestsp{f}(tempcell,6);
    %signal1(tempcell,f) = bestsp{f}(tempcell,4)./bestsp{f}(tempcell,3)/500;
    %signal1(tempcell,f) = bestsp{f}(tempcell,4);
    %signal1(tempcell,f) = bestsp{f}(tempcell,3).*bestsp{f}(tempcell,4);
    
    signal2(tempcell,f) = bestsp{f}(tempcell,6);
    %signal2(tempcell,f) = bestsp{f}(tempcell,4);
    %signal2(tempcell,f) = bestsp{f}(tempcell,3)-bestsp{f}(tempcell,4)/500;
    %signal2(tempcell,f) = bestsp{f}(tempcell,11);                           %range_of_segs/med_of_segs
    %signal2(tempcell,f) = bestsp{f}(tempcell,9)./bestsp{f}(tempcell,7);
    %signal2(tempcell,f) = bestsp{f}(tempcell,6)/300;
    %signal2(tempcell,f) = bestsp{f}(tempcell,4)/200;
    
    nucsig(tempcell,f) = bestsp{f}(tempcell,3);
    %signal3(tempcell,f) = bestsp{f}(tempcell,4)/120;
...
for f=1:totalframes
    ...
    if immunostain==0
        ...
        %allxorg(1:size(bestsp{f},1),f) = bestsp{f}(:,1); %undo get_alignment_shift
        %allyorg(1:size(bestsp{f},1),f) = bestsp{f}(:,2);
    end 
end
allx = allxorg;
...
for counter=1:length(tc_sel)
    ...
    if screening
        selection = enoughhistory & ~errantmitosis & selectedin;
    ...
    if selection
        ...
        if plotsignal2
            ...
            %line(xtime,ysig1,'DisplayName',num2str(cellid),'color',[.49 1 .63],'linewidth',signalwidth);
            %axis([0 totalframes/framesperhr ymin1 ymax1]);
            axis([0 totalframes ymin1 ymax1]);
        end
        if plotsignal3
