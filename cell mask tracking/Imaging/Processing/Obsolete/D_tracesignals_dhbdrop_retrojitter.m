...
ymin2 = 0; ymax2 = 1; ystep2 = 0.4;   %FRET signal
...
for f=1:totalframes
    ...
    allxorg(1:size(bestsp{f},1),f) = bestsp{f}(:,1) - x(f);
...
for i = leavein
    ...
    while best_rc(temptrace,5)~=best_rc(temptrace,2)
        ...
        for f=motherfirst:daughterfirst-1
            allx(i,f) = bestsp{f}(mother,1) - x(f);
            ally(i,f) = bestsp{f}(mother,2) - y(f);
        end
        temptrace = mother;
    end
...
for i = leavein
    ...
    for tt = 1:scanwidth
        ...
        nucsigdrop = [nucsigdrop,tracklengthmod(diff(angieratio(i,tracklengthmod))<-0.35)]; %E_05_1
    end
...
for counter=1:length(tc_sel)
    ...
    if screening
        ...
        selection = enoughhistory & ~errantmitosis & ~corrupted & selectedin;
