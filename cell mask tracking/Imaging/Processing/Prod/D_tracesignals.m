...
ymin2 = 0; ymax2 = 0.5; ystep2 = 0.05;
historycutoff = 1;  %fraction of trace that must exist
screening = 1;  %screen out short traces, etc.
dotsize = 8;
...
for f = 1:size(signal1,2)
    ...
    signal3(tempcell,f) = bestsp{f}(tempcell,3).*bestsp{f}(tempcell,4)/120; %additional data slot
    angieratio(tempcell,f) = bestsp{f}(tempcell,7)./bestsp{f}(tempcell,5); %always angieratio
end
...
for counter=1:length(tc_sel)
    ...
    selection = 1;
    if screening
        %selection = enoughhistory & ~errantmitosis & ~selectedout & selectedin & ~corrupted;
