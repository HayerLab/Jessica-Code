...
drugspike=39; %3C_CDK4iWee1i:14 %CyclingCDK4iWee1iCdc25i:59
minlengthtrace=15; %CDK2fatecall-minimum=15 Geminin-minimum=10 Pipdeg:5
minlengthmother=0; %PipDeg:50
framesperhr=5;
ymin=0.3; ymax=2; ystring='CDK2 activity';
%ymin=0; ymax=1; ystring='Geminin';
%ymin=0.3; ymax=1.2; ystring='Mass';
...
%%% gate CDK2 data %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
nucchannel=7; cytochannel=13; %6 8
...
%%% gate other data %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% traces1=tracedata(:,:,3);
% badtraces1=zeros(size(traces1,1),1);
...
if IFoption
    %IFvals=IFdata(:,11); badtracesIF=IFvals<1; IFvals=log2(IFvals);
    IFvals=IFdata(:,11)./IFdata(:,10); badtracesIF=IFvals<-1;
    %IFvals=IFdata(:,10).*IFdata(:,3); badtracesIF=IFvals<1; IFvals(IFvals<1)=1; IFvals=log2(IFvals);
end
%%% combine gateout %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
traces3=normalizetraces_3(traces3,tracestats,normoption);
%%% categorize traces %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[POI_Cdk2,badtraces_Cdk2]=GetPointsOfInterest_4('Cdk2',traces1,tracestats,minlengthtrace,traces2,traces3);
...
%badtracesallPOI=tracestats(:,3)>20;
%badtracesallPOI=badtraces_Cdk2 | POIcalc>-2 | tracestats(:,2)<47;
%badtracesallPOI=badtraces_Cdk2 | POIcalc>-10 | tracestats(:,2)<47;

%badtracesallPOI=badtraces_Cdk2 | POIcalc>-15 | tracestats(:,2)<80 | traces1(:,drugspike)<.8 | traces1(:,drugspike)>1;

%badtracesallPOI=badtraces_Cdk2 | POIcalc>10;
...
%%% plot data %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%hist(IFvals,0:0.05:2.5); xlim([0 2.25]); xlabel('pRb/tRb'); ylabel('# cells'); set(gcf,'color','w','PaperPosition',[0 0 4 4]); saveas(gcf,'h:\Downloads\Fig.jpg');
%dscatter(IFvals,tracesgated(:,end)-tracesgated(:,end-5));
% dscatter(IFvals,tracesgated(:,end));
% axis([-.2 2.5 0.2 2.0]); xlabel('pRb/tRb'); ylabel('DHB ratio');
% axis([-.2 2.5 -0.5 0.5]); xlabel('pRb/tRb'); ylabel('DHB ratio (F51-F46)');
% IFlow=IFvals<1; IFhigh=IFvals>=1;


%0.2-0.5
% nonbifurc=tracesgated(:,drugspike)<0.5;
% binranges=0.2:0.1:1;
% bincounts=histc(tracesgated(nonbifurc,44),binranges);
% bar(binranges,bincounts,'histc'); xlim([0.2 1]); %set(gca,'xtick',binranges);
% xlabel('DHB ratio 4hrs after spike'); ylabel('# cells');
% set(gcf,'color','w','PaperPosition',[0 0 4 2]); saveas(gcf,'h:\Downloads\Fig.jpg');

%0.5-0.8
% bifurc=tracesgated(:,drugspike)>0.5 & tracesgated(:,drugspike)<0.8;
% binranges=0.2:0.1:1.4;
% bincounts=histc(tracesgated(bifurc,44),binranges);
% bar(binranges,bincounts,'histc'); xlim([0.2 1.4]); %set(gca,'xtick',binranges);
% xlabel('DHB ratio 4hrs after spike'); ylabel('# cells');
% set(gcf,'color','w','PaperPosition',[0 0 4 2]); saveas(gcf,'h:\Downloads\Fig.jpg');
% above1=tracesgated(bifurc,44)>1;
% prcabove1=sum(above1)/sum(bifurc)

%dscatter(tracesgated(:,drugspike),tracesgated(:,end)); xlabel('DHB ratio at spike'); ylabel('DHB ratio 5hrs after spike');
%scatter(tracesgated(:,drugspike),tracesgated(:,29),'filled'); xlabel('DHB ratio at spike'); ylabel('DHB ratio 1hr after spike');
%scatter(tracesgated(:,drugspike),tracesgated(:,34),'filled'); xlabel('DHB ratio at spike'); ylabel('DHB ratio 2hrs after spike');
%scatter(tracesgated(:,drugspike),tracesgated(:,39),'filled'); xlabel('DHB ratio at spike'); ylabel('DHB ratio 3hrs after spike');
% scatter(tracesgated(:,drugspike),tracesgated(:,44)); xlabel('DHB ratio at spike'); ylabel('DHB ratio 4hrs after spike');
% 
% %hist(POIcalc/framesperhr,10);
% %dscatter(tracesgated(:,drugspike-1),tracesgated(:,drugspike+10));
% hold on; line([0 2],[0 2],'color','r','linewidth',3);
% axis([0.2 1.2 0.2 1.2]); axis([0.2 1.3 0.2 1.3]); axis([0.2 1.4 0.2 1.4]);
...
plottraces_3(aligneddata,alignedtime,xstring,ystring,[ymin ymax],tracesmoothoption,drugspike/framesperhr,tracestats); %plot traces without categorization
...
markoption=0; %1:overlay Cdk2start 2:don't overlay marks
