...
if stairview || timeview || correlationview
    stairfig = figure;
...
for i = 1:condnum
    ...
    [mitosis,g1length,g1riselength,quiescent] = getG1length_rise_includequiescent(row,col,g1max*framesperhr,drugspike);
    ...
    if timepercentview
        ...
        for cc=1:tbinnum
            ...
            binqper(cc)=numquiescent/numnormal;
            avgvals(tcidx)=tcavg;
        end
    ...
    if correlationview
        b1min = 0; b1max = 15;
        b2min = 0; b2max = 15;
        subaxis(2,3,i,'ML',0.1,'MR',0.05,'MT',0.05,'MB',0.1,'SV',0.1); %Cdt 3x4
