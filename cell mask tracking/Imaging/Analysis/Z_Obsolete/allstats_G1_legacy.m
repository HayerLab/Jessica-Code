...
if stairview || scatterview
    ...
    %set(stairfig,'Position',[round(0.4*screenx) round(0.3*screeny) round(0.6*screenx) round(0.4*screeny)]); %1x3
...
for i = 1:condnum
    ...
    cd ..\Analysis
    %[firstmitosis,g1length,g1lengthlineage,sg2length,angieg1,angiemean] = cdt1postdrugrepeat_pastcorrelate_test(row,col,bmax);
    %[mitosis,g1length] = getG1length(row,col,bmax*framesperhr,drugspike);
    [mitosis,g1length,g1riselength] = getG1length_rise(row,col,g1max*framesperhr,drugspike);
    
    %%% value selection %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %valuesfn = mitosis(:)/framesperhr;
    values = g1length(:)/framesperhr;
    %values = g1droplength(:)/framesperhr;
    %values = g1botlength(:)/framesperhr;
    ...
    if stairview
        ...
        if i==1
            ...
            %subaxis(1,2,i-1,'SV',0.05,'ML',0.05,'MR',0.05,'PB',0.05); %Cdt 3x1
            %subaxis(1,3,i-1,'SV',0.05,'ML',0.05,'MR',0.05,'PB',0.05); %Cdt 3x1
            subaxis(2,3,i-1,'ML',0.05,'MR',0.05,'MT',0.01,'MB',0.03); %Cdt 3x2
