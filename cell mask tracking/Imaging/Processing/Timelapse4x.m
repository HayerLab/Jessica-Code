function timelapsesetup3(row,col,site)
%row=3;col=5;site=1; %2 2 4
%%% set paths %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
projectpath='E:\Jessica\raw';
%imagepath='E:\Jessica\segmentation\3_8_1\Composites\aligned';
experimentpath='\160519_CDH5_mCitrine_Hoechst_1min_interval_20x\';
...
if ~exist([datadir,'tracedata_',shot,'.mat'],'file'); % makes sure analsis is run only for wells that haven't been analyzed yet
    ...
    if separatedirectories==1
        ...
        maskdir=[projectpath,experimentpath,'Mask\',shot];
        %maskdir=[imagepath,experimentpath,'Mask\'];
    else
        rawdir=[projectpath,experimentpath,shot,filesep, shot,'_']; 
        %rawdir=[imagepath,experimentpath,'Raw\',shot,'_'];
        maskdir=[projectpath,experimentpath,'Mask'];
    end
    maskwrite=1;
    ...
    jitters=zeros(EF,2);
    blocksize=10000; %% AH changed form 10000 140703, crashed in line 180 otherwise
    maxcellnum=blocksize;
    tracedata=ones(maxcellnum,EF,7)*NaN; %default 7
    ...
    %blurthreshlow=0;
    %%% shading correction %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     load([bgpath,filesep,'IXM_cmosoffset.mat']); bgcmos=cmosoffset;
%     sc1=double(imread([bgpath,filesep,'AVG_bgDAPI.tif']))-cmosoffset;
    %sc2=double(imread([bgpath,filesep,'AVG_bgYFP.tif']))-cmosoffset;
    %sc3=double(imread([bgpath,filesep,'AVG_bgRFP.tif']))-cmosoffset;
    ...
    %     real3=bgsubmasked_4(blur3,nanmask,1,compression);
        ...
        nuc_info=struct2cell(regionprops(nuc_mask,real1,'Area','Centroid','MeanIntensity','Circularity','Eccentricity','Perimeter'));
        ...
        nuc_circ=squeeze(cell2mat(nuc_info(4,1,:)));
        nuc_eccen=squeeze(cell2mat(nuc_info(5,1,:)));
        nuc_perim=squeeze(cell2mat(nuc_info(6,1,:)));
        %%% calculate masses %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        nuc_mass=nuc_density.*nuc_area;
        curdata=[nuc_center(:,1),nuc_center(:,2),nuc_area,nuc_mass,nuc_circ,nuc_eccen,nuc_perim];
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if i>firstgoodindex
            %%% calculate jitter %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if ringcalc==1
            ...
            tracedata(cellid,f,:)=[nuc_center(:,1),nuc_center(:,2),nuc_area,nuc_mass,nuc_circ,nuc_eccen,nuc_perim];
        end
        if maxcellnum-max(cellid)<blocksize
            tempdata=ones(blocksize,EF,7)*NaN; %default 7
