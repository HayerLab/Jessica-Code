...
%%% gate traces %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
firstframe=1; lastframe=20; %0719:216 0920:226 CycD-FKBP:208 SS0222:149
...
signal2=gatedtraces(:,xsig,6)./(max(gatedtraces(:,xsig,6),[],2)*ones(1,lastframe-firstframe+1));
%%% plotting %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if selectmode
        ...
        while response                                          %get next graph: 'enter'
            ...
            if strcmp(response,'2')                             %view ctd sensor: 'c','enter'
                delete(findall(gcf,'Type','hggroup'));          %must remove datapoints before changing program
                set(dcm_obj,'Updatefcn',@datatip_image_channel2_test);
                while ~strcmp(response,'d')                     %exit image mode:'d','enter'
                    response = input('','s');
                end
                delete(findall(gcf,'Type','hggroup'));
