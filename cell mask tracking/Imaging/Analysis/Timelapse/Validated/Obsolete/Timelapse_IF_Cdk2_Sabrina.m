...
if timesincerisegraph
    ...
    bincurveshade(timesince_Cdk2act/framesperhr,IF_Cdk2act,binstep,binmin,binmax,'b');
    %scatterandfit(timesince_Cdk2act/framesperhr,IF_Cdk2act,'b','r',1,binmin);
    title('IF levels vs time since Cdk2 activation');
    xlabel('Time since Cdk2 activation (hrs)'); ylabel('IF (log2(medianRFU))');
    xlim([binmin 20]);
...
if actvsnonactgraph
    ...
    title('IF levels with or without Cdk2 activation');
    ylabel('IF (log2(medianRFU))');
    set(gcf,'color','w','PaperPosition',[0 0 3 3]); %saveas(gcf,'h:\Downloads\Fig4.jpg');
end
incvslowgraph=0; %gate for daughters only (in main function below)
if incvslowgraph
    figure, hold on;
    binstep=1; binmin=minlength/framesperhr; binmax=20;
    bincurveshade(time_Cdk2inc/framesperhr,IF_Cdk2inc,binstep,binmin,binmax,'b');
    bincurveshade(time_Cdk2low/framesperhr,IF_Cdk2low,binstep,binmin,binmax,'r');
    ...
    xlabel('Time since Mitosis (hrs)'); ylabel('IF (log2(medianRFU))');
    title('IF levels for Cdk2-inc vs Cdk2-low cells');
    ...
    title('IF levels for Cdk2-inc vs Cdk2-low cells');
    ylabel('IF (log2(medianRFU))'); %ylim([7 12]);
